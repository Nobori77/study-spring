...
//    순수데이터(JwtTokenDTO) 반환
    ...
    public JwtTokenDTO login(MemberDTO memberDTO) {
        ...
        MemberDTO foundMember = memberDAO
                .findMemberByMemberEmail(memberDTO)
                .orElseThrow(() -> {
    ...
    public JwtTokenDTO socialLogin(MemberDTO memberDTO) {
        ...
        if(memberDAO.existsMemberByMemberEmail(memberDTO)){
            ...
            MemberDTO foundMember = memberDAO
                    .findMemberByMemberEmail(memberDTO)
                    .orElseThrow(() -> { throw new MemberException("social 회원 조회 실패", HttpStatus.BAD_REQUEST);});
...
//        claims.put("id", memberDTO.getId().toString());
